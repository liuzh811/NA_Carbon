...
rmse <- function(error){sqrt(mean(error^2))} #error is the residual (observed - predicted value)
...
reg.abs.mod.pred1.er.model.coef = rbind(reg.abs.mod.pred1.er.model.coef, coef(lm1.er))
...
lines(reg.abs.mod.pred.gpp[[i]][,3]+dat.df.annual.mean$prep[i], reg.abs.mod.pred.gpp[[i]][,4]+dat.df.annual.mean$gpp[i],lwd = 1.5, col=3)
...
lines(reg.abs.mod.pred.er[[i]][,3]+dat.df.annual.mean$prep[i], reg.abs.mod.pred.er[[i]][,4]+dat.df.annual.mean$er[i],lwd = 1.5, col=3)
