...
for (i in 1:11){
er1[[i]] <- focal(trendy.er.mean[[i]], w=matrix(1/25, nc=5, nr=5))
gpp1[[i]] <- focal(trendy.gpp.mean[[i]], w=matrix(1/25, nc=5, nr=5))
...
boot.rs1 = function(dat.gpp, dat.temp, dat.prep, ln = 9, n = 30){ 
...
#change to raster
er.tempr = Point2raster(er.temp, raster = gpp1[[1]])
gpp.tempr = Point2raster(gpp.temp, raster = gpp1[[1]])

er.prepr = Point2raster(er.prep, raster = gpp1[[1]])
gpp.prepr = Point2raster(gpp.prep, raster = gpp1[[1]])

gpp.rr = Point2raster(gpp.r, raster = gpp1[[1]])
er.rr = Point2raster(er.r, raster = gpp1[[1]])
...
png("F:/zhihua/dataset/results2/contributionTP.trendy.png",height = 2500, width = 2500, res = 300, units = "px")						   
